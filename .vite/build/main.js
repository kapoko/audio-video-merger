...
const ffmpeg__namespace = /* @__PURE__ */ _interopNamespaceDefault(ffmpeg);
...
const enableDevTools = !require$$3.app.isPackaged;
main.initialize();
const createWindow = () => {
...
function getFileInfo(path2) {
  const { size } = fs__namespace.statSync(path2);
  const type = String(mimeTypes.lookup(path2));
  const file = { size, path: path2, type };
  return file;
}
const debouncedOpenFiles = debounce$1(handleOpenFiles, 1e3);
require$$3.app.on("open-file", (event, path2) => {
  event.preventDefault();
  const file = getFileInfo(path2);
  openFiles.push(file);
  if (mainWindowReady) {
...
if (ffmpegPath) {
  ffmpeg__namespace.setFfmpegPath(ffmpegPath.replace("app.asar", "app.asar.unpacked"));
}
ffmpeg__namespace.setFfprobePath(
  ffprobePath.path.replace("app.asar", "app.asar.unpacked")
);
function processVideo(options, totalBytesProcessed, totalBytes, event) {
  return new Promise((resolve, reject) => {
    let duration;
    ffmpeg__namespace.default().input(options.video.path).addInput(options.audio.path).outputOptions(["-c:v copy", "-map 0:v:0", "-map 1:a:0"]).on(
      "codecData",
      (data) => duration = data.video_details ? getSeconds(data.duration) : false
    ).on("end", resolve).on("error", (error) => reject(error.message)).on("progress", (progress) => {
...
require$$3.ipcMain.on("merge", async (event, input) => {
  ...
  audioList.forEach((audio) => {
    const filename = path__namespace.basename(audio.path, path__namespace.extname(audio.path));
    if (videoList.map((video) => path__namespace.basename(video.path, path__namespace.extname(video.path))).includes(filename)) {
      require$$3.dialog.showMessageBoxSync(mainWindow, {
        type: "error",
        message: `Trying to convert ${path__namespace.basename(audio.path)} but a video with the same name is being used as a source file. Please rename the source video.`
      });
  ...
  videoList.forEach((video) => {
    audioList.forEach((audio) => {
      const dir = path__namespace.dirname(audio.path);
      const fileName = path__namespace.basename(audio.path, path__namespace.extname(audio.path)) + path__namespace.extname(video.path);
      const videoBaseName = path__namespace.basename(video.path, path__namespace.extname(video.path));
      const output = videoList.length > 1 ? path__namespace.join(dir, videoBaseName + "_" + fileName) : path__namespace.join(dir, fileName);
      if (fs__namespace.existsSync(output) && !yesToAll && !noToAll) {
        if (path__namespace.basename(video.path) === path__namespace.basename(output)) {
          console.log("yeaa");
        }
        const result2 = require$$3.dialog.showMessageBoxSync(mainWindow, {
          type: "question",
          message: "The file " + path__namespace.basename(output) + " already exists in the source folder. Overwrite it?",
  ...
  const totalBytes = processChain.reduce(
    (total, process2) => total += process2.bytes,
    0
  );
...
require$$3.ipcMain.on("showDialog", (event, options) => {
  require$$3.dialog.showMessageBox(mainWindow, options);
});
require$$3.ipcMain.on("showOpenDialog", (event, options) => {
  const filePaths = require$$3.dialog.showOpenDialogSync(
    mainWindow,
    options
  );
  if (filePaths) {
    event.reply(
      ...
      filePaths.map((path2) => getFileInfo(path2))
    );
  } else {
